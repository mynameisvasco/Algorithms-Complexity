...
{
  Graph *gCopy = GraphCreate(g->numVertices, g->isDigraph, g->isWeighted);
  for (int i = 1; i < gCopy->numVertices; i++)
  {
    ListMove(g->verticesList, i);
    ListMove(gCopy->verticesList, i);
    ListInsert(gCopy->verticesList, ListGetCurrentItem(g->verticesList));
  }
  return g;
}
