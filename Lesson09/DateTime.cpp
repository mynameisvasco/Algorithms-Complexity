...
DateTime *DateTimeCreate(int yy, int mm, int dd, int h, int m, int s)
{
  ...
  datetime->date = *DateCreate(yy, mm, dd);
  datetime->time = TimeCreate(h, m, s);
  return datetime;
}
...
int DateTimeCompare(DateTime *a, DateTime *b)
{
  ...
  const Date *d1 = DateCreate(a->date.year, a->date.month, a->date.day);
  const Date *d2 = DateCreate(b->date.year, b->date.month, b->date.day);
