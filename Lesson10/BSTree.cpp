...
static int _treeGetHeight(const struct _BSTreeNode *root)
{
  ...
  int lheight = _treeGetHeight(root->left);
  int rheight = _treeGetHeight(root->right);
...
// R: A complexidade computacional da função getPointerToKthNode é O(n) pois
// vai aumentar linearmente com o número de nodes da árvore
//
